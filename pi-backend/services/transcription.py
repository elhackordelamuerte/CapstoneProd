...
class TranscriptionService:
    ...
    async def _run_whisper(self, audio_path: str) -> str:
        ...
        if proc.returncode != 0:
            stderr_out = stderr.decode(errors="replace").strip() if stderr else ""
            raise RuntimeError(
                f"Whisper exited with code {proc.returncode} for {audio_path}: {stderr_out}"
            )

        txt_path = audio_path + ".txt"
        if not os.path.exists(txt_path):
            stderr_out = stderr.decode(errors="replace").strip() if stderr else "No output file"
            raise RuntimeError(f"Whisper did not produce output: {txt_path}. Error: {stderr_out}")
